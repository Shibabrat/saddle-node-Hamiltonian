...
te = soln.t_events[0]
tt = [0,te[2]]
t,x,phi_t1,PHI = saddlenode_tpcd.stateTransitMat_saddlenode(tt,x0po_1,parameters)
x_po1 = x[:,0]
y_po1 = x[:,1]
px_po1 = x[:,2]
py_po1 = x[:,3]
...
for i in range(len(x[:,3])-1):
    q1 = q1+ 0.5*(t[i+1]-t[i])*(py_po1[i+1]**2+py_po1[i]**2) + 0.5*(t[i+1]-t[i])*(px_po1[i+1]**2+px_po1[i]**2)
    q2 = q2+ 0.5*(t[i+1]-t[i])*(py_po2[i+1]**2+py_po2[i]**2) + 0.5*(t[i+1]-t[i])*(px_po2[i+1]**2+px_po2[i]**2)
    q3 = q3+ 0.5*(t[i+1]-t[i])*(py_po3[i+1]**2+py_po3[i]**2) + 0.5*(t[i+1]-t[i])*(px_po3[i+1]**2+px_po3[i]**2)
